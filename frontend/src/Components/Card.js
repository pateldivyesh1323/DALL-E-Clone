    ...
    <div className='rounded-xl group relative shadow-card hover:shadow-cardhover card'>
      ...
      <div className='group-hover:flex flex-col max-h-[94.5%] hidden absolute bottom-0 left-0 right-0 bg-[#10131f] m-2 p-4 rounded-md'>
        ...
        <div className='mt-5 flex justify-between item-center'>
          ...
          <button type='button' onClick={()=>{downloadImage(_id,photo)}} className='outline-none bg-transperane border-none'><img src={download} alt="download" className='w-6 h-6 object contain invert'/></button>
        </div>
      </div>
    </div>
  )
